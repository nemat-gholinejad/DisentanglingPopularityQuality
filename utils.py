...
def get_items_popularity(edge_index, top_rated=0.2):
    ...
    return popular_items, long_tails, dict(zip(item_count[:, 0], item_count[:, 1] / item_count.shape[0])), dict(
        zip(item_count[:, 0], item_count[:, 1]))
...
class Monitor:
    ...
    def coninue_check(self, epoch, metric_value, **kwargs):
        ...
        if self.is_better(metric_value):
            ...
            # self.best_weight_decay = self.optimizer['weight_decay']
