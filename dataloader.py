...
def load_node_csv(path: str) -> dict:
    ...
    item_mapping = {index: i for i, index in enumerate(np.unique(trainItem))}
    start_index = len(item_mapping)
    user_mapping = {index: i + start_index for i, index in enumerate(uids)}

    return user_mapping, item_mapping


# load edges between users and items
def load_edge_csv(path: str, users_mapping, items_mapping):
    ...
    R_edge_index = np.array([user, item])
    undirect_ui_edge_index = np.concatenate((R_edge_index, np.flip(R_edge_index, axis=0)), axis=1)


    return torch.as_tensor(R_edge_index).to(torch.long), torch.as_tensor(undirect_ui_edge_index).to(torch.long)
